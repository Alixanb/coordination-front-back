...
Le backend **émet et valide ses propres JWT** (RS256). Le endpoint `POST /token` (HTTP Basic) délivre un jeton signé avec une paire RSA générée par l'application ; le même service agit comme _OAuth2 Resource Server_ pour valider ce jeton à chaque requête.
